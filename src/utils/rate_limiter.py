...
class RateLimiter:
    ...
    def __init__(self, custom_limits: Optional[Dict] = None):
        ...
        logger.info(f"[RateLimiter] Initialized with {len(self._limits)} domain configurations")
        ...
        with self._lock:
            ...
            if last > 0:
                ...
                if elapsed < min_delay:
                    ...
                    logger.debug(f"[RateLimiter] {domain}: waiting {wait_time:.2f}s for min_delay")
                    time.sleep(wait_time)
            
            # Clean old request timestamps (older than 60 seconds)
            self._request_counts[domain] = [
                ts for ts in self._request_counts[domain]
                if now - ts < 60
            ]
            ...
            if len(self._request_counts[domain]) >= rpm_limit:
                ...
                if wait_time > 0:
                    logger.warning(f"[RateLimiter] {domain}: RPM limit ({rpm_limit}) reached, waiting {wait_time:.2f}s")
                    time.sleep(wait_time)
        ...
        with self._lock:
            ...
            for domain in self._request_counts:
                ...
                stats[domain] = {
                    ...
                    "last_request_ago": now - self._last_request[domain] if self._last_request[domain] else None,
                }
            return stats
